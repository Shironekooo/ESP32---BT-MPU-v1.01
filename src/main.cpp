...
float SensorReadingYaw, SensorReadingPitch, SensorReadingRoll;
float Flexangle;
int   flexresistance;
...
void setup()
{
  flexresistance = analogRead(FLEX_SENSOR_PIN);
  initMPU();
  initBLE();
}
...
void GyroSensorReadings()
{
  ...
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer))
  {
    ...
    SensorReadingRoll = ypr[1] * 180 / M_PI;
    SensorReadingPitch = ypr[2] * 180 / M_PI;
  }
}

void FlexSensorReadings()
{
  int flexval = analogRead(FLEX_SENSOR_PIN);
  Flexangle = map(flexval,flexresistance,flexresistance-530,90,0);
}
...
void NotifyDevice()
{
  ...
  dtostrf(Flexangle, 6, 2, Flex);

  // final result
  String result = String(Pitch) + "," + String(Flex);
